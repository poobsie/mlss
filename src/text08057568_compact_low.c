...
struct CompactLowObject {
    u8 unknown_00[8];
    struct CompactLowNested *nested;
    u8 unknown_0C[0x40];
    void (*callback)(void *);
    u8 unknown_50[0x4C];
    s32 timer;
...
#define DEFINE_SETUP(name, kind, next)                                     \
    ...
    void name(struct CompactLowObject *) FUNCTION_SECTION(name);           \
    void name(struct CompactLowObject *object)                             \
    {                                                                      \
