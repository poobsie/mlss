...
SEC(script_cmd_return) int script_cmd_return(void* unused, u8* state) {
    if (U32AT(state, 0x10) == 0)
        U32AT(state, 4) = U32AT(state, 0);
    else {
        U32AT(state, 0) = U32AT(state, 0x10);
        U32AT(state, 0x10) = 0;
    }
    return 1;
}
...
SEC(sub_8134B24) int sub_8134B24(u16* p) {
    return p[1] >= p[2];
}
...
SEC(sub_81251DC) void* sub_81251DC(void) {
    return (u8*)**(void***)0x03000FFC + 0x1FF8;
}
...
SEC(sub_80F7D64) int sub_80F7D64(u8* p) {
    U8AT(PTRAT(p, 0x2C), 0x179D) = (U8AT(PTRAT(p, 0x2C), 0x179D) & ~3) | 1;
    return 1;
}
...
SEC(sub_80EB048) int sub_80EB048(void* a, void* b, u32* args) {
    U8AT(**(u8***)0x03000FB8, 0x32) = args[0];
    return 1;
}
