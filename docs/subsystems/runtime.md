...
`runtime_dispatch_frame_callbacks` is the tick function installed by the adjacent
callback service initializer. It invokes the first two entries in the callback
array at `0x03000D80` when they are non-null. The setter and remaining array
capacity stay in assembly, so the API claims only the two observed dispatch
slots.
...
`runtime_intrusive_list_detach` now owns the complementary unlink operation. It repairs both neighboring links, updates the head or tail when detaching an endpoint, and decrements the count. It deliberately leaves the detached node intact because the routine performs no destruction or field clearing.

The runtime bitset at `0x03000FC0` exposes one test and one assignment operation. A 16-bit bit number selects a word after the owner's unknown 32-byte prefix and a bit within that word. The owner remains generic because current callers establish packed flag storage, but not the meaning of individual flags.
...
`ui_object_visual_is_idle` now exposes the shared readiness predicate used by the
same object family. A missing visual or state value 1 is considered idle; the
concrete visual payload and other state values remain unspecified.
