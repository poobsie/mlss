...
The adjacent scene callbacks are now ordinary typed C. `sub_8046980` rounds the four observed fixed-point position words at `0x0C`, `0x10`, `0x14`, and `0x18` to 256-unit boundaries when the low scene-state group is active, conditionally includes the latter pair, clears `0x2E0` and movement deltas, and removes the active low bits. `sub_80492DC` selects the proven `0x103E` or `0x1053` mode values at `0x218`, applies the corresponding `0x1001`, `0x1028`, or `0x1002` transition through `sub_8049000`, and clears bit `0x02` at `0x35A`. The mode byte at `0x54`, selector, and flags remain structural because their gameplay identities are not established.
...
The paired visual-timer callbacks decrement the signed timer at `0x10`. At zero they
...
A related countdown callback decrements the same signed timer before testing it. At
...
The display-value callback derives its horizontal position from runtime byte
`0x342`, draws the signed value at transition offset `0x16`, and waits for flag
`0x08` on the linked visual before destroying it and advancing the callback.

Its adjacent step callback passes three times the signed step at `0x14` to the
...
`FieldObjectSetDirectory` describes the eight-byte directory embedded in each field-object definition blob. Its packed bytes expose the object count and resource count, while backward offsets locate the blob's variable-length lists. `field_get_object_resource_list` now returns the resource-ID list used by the early field-object loader instead of presenting the calculation as an unrelated numbered helper. `field_load_object_resource_handles` retains that list, temporarily clears the loader callback slot, and stores one loaded handle per ID before restoring the callback. `field_get_object_auxiliary_list` decodes the five-bit count split across the two packed count bytes and follows the middle back-offset. Its direct consumer, `field_load_auxiliary_resource_handles`, passes each 16-bit entry to the other resource-loader entry point and retains the returned handle in a separate parallel array. `field_release_inline_resource_objects` uses the independently guarded count at offsets `0x112` and `0x113` to release the inline object array beginning at `0x30`. `field_destroy_owned_objects_and_allocations` covers the broader teardown: each non-null object in the `0x28` array supplies a signed owner adjustment and destructor callback, then the eight allocations at `0xA8` are released through their original heap entry point. The resources' content roles remain unknown, so `auxiliary` is intentionally structural. Two parallel indexed pointer tables remain `table_a` and `table_b`: their contents are proven to be metadata streams, but no decompiled caller distinguishes their roles yet.
...
`field_apply_indexed_scene_setup` forwards one byte-sized selection to three
ordered scene-setup stages. Current callers establish the shared context and
`FieldSceneObject` arguments, but the selection's gameplay identity remains
unknown.

Field scene collision checks now include
`field_scene_vertical_ranges_overlap`. Each object's vertical range extends
from `positionY - verticalExtent276` through `positionY`, inclusive. The field
at `0x276` is therefore named for its proven geometric role; its source data
and gameplay meaning remain unknown.

`field_forward_linked_callback_and_advance_on_sprite_flag` invokes the linked
object's active chain callback when present, passing that callback slot so the
callee may replace it. It then tests the current sprite's established bit 3 at
offset `0x12`; when set, it selects low mode value 3 and installs the next
dispatcher. The condition is explicit in canonical assembly, with no
uninitialized input.
...
and show. `field_script_mark_return_pending` sets shared byte `0x9A1` to one
before returning from the current script channel.

Adjacent commands expose two channel-launch paths. One starts the primary channel
owned by the global field-script runtime at offset `0x1F8`; the other starts the
root channel owned by the command runtime and records value two at shared byte
`0x9A1`. Both use proven flag `1` and mode `0xFF`. An indexed command also sets or
...
`field_script_update_channels` exposes the caller that keeps the channel array
live. It ticks the root channel when flag `0x0010` is set, then visits the twelve
indexed channels. Channels masked by `0x0084` are skipped; the remaining flag
...
offset `0xF0`. For each set bit, it copies one `0x20`-byte block from
`workspaceBlocks24` into the mirror beginning at `0x02000080`, then shifts the
mask until it is cleared. `field_restore_workspace_blocks_and_mark_dirty`
performs the inverse copy for a supplied 32-bit block mask and marks every
restored block dirty. The higher-level role of the workspace and mirror remains
unknown.
