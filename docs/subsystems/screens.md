...
The shop screen teardown is isolated in `src/screens/shop_runtime.c`. It restores the installed process definition, clears the shared work-buffer control halfwords, releases the screen's loader and workspace, and frees the mode-dependent buffer set before removing its auxiliary and child processes. The constructor proves the ownership slots and the two layout controls; individual buffer roles retain offset-based names because their data formats are not yet established.
