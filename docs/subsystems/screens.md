...
Late screen helpers now live in `src/screens/runtime_helpers.c`. The process launchers retain their ROM allocation labels, allocation sizes, and distinct constructors. The layer helpers use the shared hardware-backed configuration routine, including variants that mark a proven completion byte, copy a control halfword, fade or start music, or reset the shared runtime field before applying transfer parameters. Their pollers report an active value transfer and clear the matching marker at offset `0x69`, `0xEF`, or `0x156` when it finishes; the script wait handler uses the `0x156` owner variant. These numeric names remain until callers establish what each marker represents. The sparse owner layouts remain separate because matching field offsets alone does not prove a common screen class.

The shop screen lifecycle is isolated in `src/screens/shop_runtime.c`. Its teardown restores the installed process definition, clears the shared work-buffer control halfwords, releases the screen's loader and workspace, and frees the mode-dependent buffer set before removing its auxiliary and child processes. The compact child update follows the process definition's parent link, optionally refreshes the owner when its offset-based update flag is set, then runs the shared display callback chain and stores the resulting display state. The constructor proves the ownership slots and the two layout controls; individual buffer roles retain offset-based names because their data formats are not yet established.
